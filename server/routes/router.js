...
// this start
const multipleUpload = upload.fields([{ name: 'fileCSF', maxCount: 1 }, 
    { name: 'fileCIF', maxCount: 1 },
    { name: 'fileCartaConformidad', maxCount: 1 },
    { name: 'fileCotizacion', maxCount: 1 },
    { name: 'fileFactura', maxCount: 1 },
    { name: 'fileOrdenCompra', maxCount: 1 },
    { name: 'fileOtro', maxCount: 1 },])

router.post('/project', multipleUpload, (req, res) => {
// router.post('/project', upload.array('file', 10), (req, res) => {
    ...
    projectModel.create({...req.body, files: req.files})
    .then((data) => {
        res.send({data:data,message:"Proyecto creado exitosamente"})
        console.log(data);
        console.log("Project created!");
...
//API || ENDPOINT FOR POSTING A NEW TASK
// NEW JAN28
router.post('/tasker', (req,res) => {
    let tasks = req.body
    console.log(tasks)
    console.log(req.body)
    ...
    .then((data) => {
        res.send({data:data,message:"Tarea creada exitosamente"})
        console.log(data);
...
router.post("/updateTask", async (req, res) => {
    // MODIF Jun/04
    const {id, prioridadTarea, nombreTarea, descripcionTarea, encargadoTarea, fechaAsignacion, clienteTarea, estatusTarea} = req.body
        console.log(req.body)
...
router.post("/updateProject", multipleUpload, async (req, res) => {
    // MODIF Jun/04
    const {id, empresa, cliente, numeroCliente, razonSocial, correoPrincipal, numeroTelefono, direccion, sucursal, ubicacion, giro,
        ...
        payment, extraPayment, item, fileCIF} = req.body
        console.log(req.body)

    const filesUpdate = req.files
