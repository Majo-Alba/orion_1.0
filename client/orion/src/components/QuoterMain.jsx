...
import qrCode from "../assets/images/Comunicaci_n_Digital_Estrella-2.png"
// end aug30
...
function QuoterMain() {
    ...
    const[quoterDetails, setQuoterDetails] = useState({
        ...
        terminosYCondiciones:"1. Es importante aclarar que los precios pueden variar sin previo aviso \n \n2. Todos los precios están expresados en ________ \n \n3. Se requiere el ___% del anticipo y ___% al finalizar a la entera satisfacción y/u Orden de Compra \n \n4. Los precios unitarios no incluyen I.V.A. \n \n5. Los precios expresados en este documento no incluyen accesorios de instalación ni mano de obra aparte del aquí cotizado \n \n6. Un año de garantía directo con Grupo Estrella",
        ...
        // new sep17
        creatorDetails:"",
        pamelaData:"Lic. Diana Pamela Flores Fonseca \n \nCel.: 331 811 3273 \n \nCorreo: pamelaflores@cdestrella.com.mx \n \nConmutador: 333 827 0808 / 333 826 2727 \n \nPto. Vallarta: 322 196 0791",
        cesarData:"Ing. César Alejandro Flores Fonseca \n \nCel.: 331 618 4753 \n \nCorreo: cesarflores@cdestrella.com.mx \n \nConmutador: 333 827 0808 / 333 826 2727 \n \nPto. Vallarta: 322 196 0791",
        gusData:"Ing. Gustavo Flores Fonseca \n \nCel.: 331 541 2236 \n \nCorreo: gustavoflores@cdestrella.com.mx \n \nConmutador: 333 827 0808 / 333 826 2727 \n \nPto. Vallarta: 322 196 0791",
        gustavoData:"Ing. Gustavo Anselmo Flores Valdivia \n \nCel.: 331 541 2237 \n \nCorreo: gf@cdestrella.com.mx \n \nConmutador: 333 827 0808 / 333 826 2727 \n \nPto. Vallarta: 322 196 0791",

        // end sep17
    })

    // new sep16
    // var creator = (quoterDetails.elaboradaPor);

    // function creatorInfo() {
    //     // var creator = (quoterDetails.elaboradaPor);
    //     if(quoterDetails.elaboradaPor === "Pamela Flores") {
    //         console.log("beeebeee")
    //         creator = quoterDetails.pamelaData
    //     }
    // }
    // var creator = (quoterDetails.elaboradaPor)
    // console.log(creator)
    ...
    var creator = (quoterDetails.elaboradaPor)
    var creatorInfo = (quoterDetails.creatorDetails);
    switch(creator) {
        case "Pamela Flores": {
            // console.log(quoterDetails.pamelaData)
            creatorInfo = (quoterDetails.pamelaData);
        break;
        }
        case "Cesar Flores": {
            console.log("cesar registrado")
            creatorInfo = (quoterDetails.cesarData);
        break;
        }
        case "Gustavo Flores": {
            // console.log(quoterDetails.pamelaData)
            creatorInfo = (quoterDetails.gusData);
        break;
        }
        case "Gustavo Valdivia": {
            console.log("cesar registrado")
            creatorInfo = (quoterDetails.gustavoData);
        break;
        }
    }

    console.log(quoterDetails.creatorDetails)
    // end sep16
    ...
    // NEW AUG30
    const [item,setItem]=React.useState([{iname:"",model:"",quantity:"",amount:"",total:0}])

    // new sep01
    const [labourItem,setLabourItem]=React.useState([{iname:"",model:"",quantity:"",amount:"",total:0}])
    const [miscItem,setMiscItem]=React.useState([{iname:"",model:"",quantity:"",amount:"",total:0}])
    // end sep01
    ...
    // const stringCreatorInfo = JSON.stringify(creatorInfo)
    const stringCreatorInfo = creatorInfo
    ...
    function handlePDFGenerator(event)
    {
        ...
        {
            newArray.push([i+1,item[i].iname,item[i].model,item[i].quantity,item[i].amount,item[i].total])
        }
        // new sep01
        for(let i=0;i<labourItem.length;i++)
        {
            newLabourArray.push([i+1,labourItem[i].iname,labourItem[i].model,labourItem[i].quantity,labourItem[i].amount,labourItem[i].total])
        }

        for(let i=0;i<miscItem.length;i++)
        {
            newMiscArray.push([i+1,miscItem[i].iname,miscItem[i].model,miscItem[i].quantity,miscItem[i].amount,miscItem[i].total])
        }
        ...
        doc.setFontSize(10);
        doc.setFont('custom', 'bold');
        doc.text(`Fecha de Elaboración: ${quoterDetails.fechaElaboracion}`, 175, 25, null, null, "right");
        doc.text(`Cotización Valida Hasta: ${quoterDetails.fechaValidez}`, 175, 30, null, null, "right");
        doc.text(`ID de Cotización: ${quoterDetails.idCotizacion}`, 175, 35, null, null, "right");
        doc.text(`Elaborada Por: ${quoterDetails.elaboradaPor}`, 175, 40, null, null, "right");
        doc.addImage(qrCode, 180, 21, 20, 25)
        ...
        doc.setFontSize(11);
        doc.setFont('custom', 'bold');
        doc.text("Información del Servicio", 13,99); +7

        doc.setFontSize(10);
        doc.text(`Cliente: ${quoterDetails.receptorServicio}`, 13, 106, null, null);+6
        doc.text(`Calle: ${quoterDetails.calleServicio}`, 13, 112, null, null);
        doc.text(`Num. Ext.: ${quoterDetails.numExteriorServicio}`, 13, 118, null, null);
        doc.text(`Num. Int.: ${quoterDetails.numInteriorServicio}`, 13, 124, null, null);

        doc.text(`Col.: ${quoterDetails.colServicio}`, 100, 106, null, null);
        doc.text(`C.P.: ${quoterDetails.cpServicio}`, 100, 112, null, null);
        doc.text(`Ciudad: ${quoterDetails.ciudadServicio}`, 100, 118, null, null);
        doc.text(`Estado: ${quoterDetails.estadoServicio}`, 100, 124, null, null);
        ...
        doc.text("Materiales", 13,162); +19 / 125
        doc.autoTable({
        // head: [['S.no','Item name', 'Model', 'Quantity','Amount','Total']],
        head: [['S.no','Material', 'Modelo', 'Cantidad','Precio Unit','Total']],
        body: newArray,
        startY: 167,
        })
        ...
        doc.autoTable({
        // head: [['S.no','Item name', 'Model', 'Quantity','Amount','Total']],
        head: [['S.no','Descripción', 'Modelo', 'Unidades','Precio Unit','Total']],
        body: newLabourArray,
        ...
        let termsY = doc.previousAutoTable.finalY;

        // doc.setFillColor(128,128,127);
        // doc.text(`${stringCreatorInfo}`, 13, termsY+ 10)
        doc.autoTable({
            body: creatorArray,
            startY: termsY+25,
            columnStyles: {
                                ...
                                <div className="locationDiv">
                                    ...
                                    <div className="descript-clientCard">
                                        <div className="locationDate-labelInput">
                                            <label>Cliente</label>
                                            <input className="quoterDescriptPlaceholer" type="text"required onChange={handleInput} placeholder="Nombre de quien recibe servicio" name="receptorServicio" value={quoterDetails.receptorServicio}></input>
                                        </div>
                                        <div className="locationDate-labelInput">
                                            <label>Calle</label>
                                ...
                                <div className="locationDiv">
                                    ...
                                    <div className="datesCard">
                                        ...
                                        <div className="dates-labelInput">
                                            ...
                                            {/* NEW SEP16 */}
                                            <select required onChange={handleInput} placeholder="Nombre de quien elabora" name="elaboradaPor" value={quoterDetails.elaboradaPor}>
                                                <option>Nombre de quien elabora</option>
                                        ...
                                        {/* NEW SEP16 */}
                                        <div className="dates-labelInput">
                                            <label>Datos de quien elabora</label>
                                            <textarea className="quoterDescriptPlaceholer" type="text"required onChange={handleInput} placeholder="Detalles de Responsable" name="creatorDetails" value={creatorInfo}>{creatorInfo}</textarea>
                                            {/* <input className="quoterDescriptPlaceholer" type="text"required onChange={handleInput} placeholder="Nombre de quien elabora" name="elaboradaPor" value={quoterDetails.elaboradaPor}></input> */}
                                        </div>
                                        {/* END SEP16 */}
                                    </div>
                                        ...
                                        <tbody onChange={handleInput} placeholder="materials" name="materials" value={quoterDetails.materials}>
                                            ...
                                            {item.map((input,index) =>(
                                                <div key={index} className="invoiceItemRow">
                                                    ...
                                                    <input className="invoiceItem-Item" name="model" placeholder='Modelo' value={input.model} onChange={event => handleChange(index,event)}></input>
                                        ...
                                        <tbody onChange={handleInput} placeholder="labour" name="labour" value={quoterDetails.labour}>
                                            ...
                                            {labourItem.map((input,index) =>(
                                                <div key={index} className="invoiceItemRow">
                                                    ...
                                                    <input className="invoiceItem-Item" name="model" placeholder='Modelo' value={input.model} onChange={event => handleLabourChange(index,event)}></input>
                                        ...
                                        <tbody onChange={handleInput} placeholder="labour" name="labour" value={quoterDetails.MISC}>
                                            ...
                                            {miscItem.map((input,index) =>(
                                                <div key={index} className="invoiceItemRow">
                                                    ...
                                                    <input className="invoiceItem-Item" name="model" placeholder='Modelo' value={input.model} onChange={event => handleMiscChange(index,event)}></input>
