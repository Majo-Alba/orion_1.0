...
function Project() {
    ...
    const [fileCSF, setFileCSF] = useState([])
    const [fileCartaConformidad, setFileCartaConformidad] = useState([])
    const [fileCotizacion, setFileCotizacion] = useState([])
    const [fileFactura, setFileFactura] = useState([])
    const [fileOrdenCompra, setFileOrdenCompra] = useState([])
    const [fileOtro, setFileOtro] = useState([])

    // END JUL18

    // ASLEEP 18 - START: DOCUMENT CHECKLIST DB
    ...
    //     { id: "7", value: "Otros"}
    // ]
    // ASLEEP 18 - END: DOCUMENT CHECKLIST DB
    ...
    //   filesArr.push(fileCIF)
    //   console.log(filesArr)
  
    //   filesArr.push(fileCSF)
    //   console.log(filesArr)
      // END OCT16
    // END OCT15
    ...
    // NEW JUL18
    // START: CIF
    const [checkedCIF, setCheckedCIF] = useState([])

    const handleSelectCIF = (event) => {
        const valueCIF = event.target.value
        const isCheckedCIF = event.target.checked

        if(isCheckedCIF) {
            setCheckedCIF([...checkedCIF, valueCIF])
        } else {
            const filteredCIF = checkedCIF.filter((item) => item != valueCIF)
            setCheckedCIF(filteredCIF)
        }
    }
    // END: CIF

    // START: CSF
    const [checkedCSF, setCheckedCSF] = useState([])

    const handleSelectCSF = (event) => {
        const valueCSF = event.target.value
        const isCheckedCSF = event.target.checked

        if(isCheckedCSF) {
            setCheckedCSF([...checkedCSF, valueCSF])
        } else {
            const filteredCSF = checkedCSF.filter((item) => item != valueCSF)
            setCheckedCSF(filteredCSF)
        }
    }
    // END: CSF

    // START: CARTA CONFORMIDAD
    const [checkedCartaConformidad, setCheckedCartaConformidad] = useState([])

    const handleSelectCartaConformidad = (event) => {
        const valueCartaConformidad = event.target.value
        const isCheckedCartaConformidad = event.target.checked

        if(isCheckedCartaConformidad) {
            setCheckedCartaConformidad([...checkedCartaConformidad, valueCartaConformidad])
        } else {
            const filteredCartaConformidad = checkedCartaConformidad.filter((item) => item != valueCartaConformidad)
            setCheckedCartaConformidad(filteredCartaConformidad)
        }
    }
    // END: CARTA CONFORMIDAD

    // START: COTIZACION
    const [checkedCotizacion, setCheckedCotizacion] = useState([])

    const handleSelectCotizacion = (event) => {
        const valueCotizacion = event.target.value
        const isCheckedCotizacion = event.target.checked

        if(isCheckedCotizacion) {
            setCheckedCotizacion([...checkedCotizacion, valueCotizacion])
        } else {
            const filteredCotizacion = checkedCotizacion.filter((item) => item != valueCotizacion)
            setCheckedCotizacion(filteredCotizacion)
        }
    }
    // END: COTIZACION

    // START: FACTURA
    const [checkedFactura, setCheckedFactura] = useState([])

    const handleSelectFactura = (event) => {
        const valueFactura = event.target.value
        const isCheckedFactura = event.target.checked
 
        if(isCheckedFactura) {
            setCheckedFactura([...checkedFactura, valueFactura])
        } else {
            const filteredFactura= checkedFactura.filter((item) => item != valueFactura)
            setCheckedFactura(filteredFactura)
        }
    }
    // END: FACTURA

    // START: ORDEN DE COMPRA
    const [checkedOrdenCompra, setCheckedOrdenCompra] = useState([])

    const handleSelectOrdenCompra = (event) => {
        const valueOrdenCompra = event.target.value
        const isCheckedOrdenCompra = event.target.checked
 
        if(isCheckedOrdenCompra) {
            setCheckedOrdenCompra([...checkedOrdenCompra, valueOrdenCompra])
        } else {
            const filteredOrdenCompra= checkedOrdenCompra.filter((item) => item != valueOrdenCompra)
            setCheckedOrdenCompra(filteredOrdenCompra)
        }
    }
    // END: ORDEN DE COMPRA

    // START: ORDEN DE COMPRA
    const [checkedOtro, setCheckedOtro] = useState([])

    const handleSelectOtro = (event) => {
        const valueOtro = event.target.value
        const isCheckedOtro = event.target.checked
 
        if(isCheckedOtro) {
            setCheckedOtro([...checkedOtro, valueOtro])
        } else {
            const filteredOtro= checkedOtro.filter((item) => item != valueOtro)
            setCheckedOtro(filteredOtro)
        }
    }
    // END: ORDEN DE COMPRA
    ...
    // const filesFinalArr = []
    // filesArr.forEach(project => {
    //     console.log(files)
    //     filesFinalArr.push(e.target.files)
    // })
    // console.log(filesFinalArr)
    // END OCT15
    ...
    function handleSubmit() {
        ...
        // console.log(projectDetails.fileCIF)
        // console.log(projectDetails.fileCSF)
        // END JUL18
        // console.log(file)
        ...
        // // formData.append('files', filesArr[1]);
        // let file = 0
        // const innerFileArr = []
        // for (let i = 0; i < filesArr.length; i++) {
        //     // formData.append('files[]', filesArr[i])
        //     // formData.append(filesArr[i].name, filesArr[i])
        //     // var file = filesArr[i];
        //     file = filesArr[i];
        //     innerFileArr.push(file)
        //     // console.log(file)
        //     console.log(('files[]', filesArr[i]))
        // }
        // console.log(innerFileArr)

        // formData.append('files', innerFileArr)
        // - OFF OCT17
        ...
        // NEW JUL22
        formData.append("empresa", projectDetails.empresa)
        // END JUL22
        ...
        // NEW JUL22
        formData.append("numeroTelefono", projectDetails.numeroTelefono)
        // END JUL22
        formData.append("direccion", projectDetails.direccion)
        formData.append("sucursal", projectDetails.sucursal)
        // NEW JUL22
        formData.append("ubicacion", projectDetails.ubicacion)
        formData.append("giro", projectDetails.giro)
        // END JUL22
        ...
        // NEW JUL18
        formData.append('fileCIF', fileCIF[0])
        formData.append('fileCSF', fileCSF[0])
        formData.append('fileCartaConformidad', fileCartaConformidad[0])
        formData.append('fileCotizacion', fileCotizacion[0])
        formData.append('fileFactura', fileFactura[0])
        formData.append('fileOrdenCompra', fileOrdenCompra[0])
        formData.append('fileOtro', fileOtro[0])
        // END JUL18
        ...
        // axios.post('http://localhost:4000/project', formData, {
            ...
            // END OCT15
            empresa: projectDetails.empresa,
            ...
            numeroTelefono: projectDetails.numeroTelefono,
            direccion: projectDetails.direccion,
            sucursal: projectDetails.sucursal,
            ubicacion: projectDetails.ubicacion,
            giro: projectDetails.giro,
            ...
            // NEW JUL18
            // fileCIF: projectDetails.fileCIF,
            // fileCSF: projectDetails.fileCSF,

            fileCIF: fileCIF,
            fileCSF: fileCSF,
            fileCartaConformidad: fileCartaConformidad,
            fileCotizacion: fileCotizacion,
            ...
            <div>
                ...
                <form action="/project" method="POST" enctype="multipart/form-data" name="myFiles" id="newProjectForm" className="container-newProject" onSubmit={handleSubmit}>
                    ...
                    <div className="conditionHeader">
                        ...
                        <select required onChange={handleInput} placeholder="Seleccionar condición..." name="condicion" value={projectDetails.condicion}>
                    ...
                    <div className="multiFieldRowThree">
                        <input type="text"required onChange={handleInput} placeholder="Ingresa nombre de empresa..." name="empresa" value={projectDetails.empresa}></input>
                    ...
                    <div className="multiFieldRowThree">
                        ...
                        <input type="text" onChange={handleInput} placeholder="Ingrese número de teléfono principal..." name="numeroTelefono" value={projectDetails.numeroTelefono}></input>
                    </div>
                    ...
                    <div className="multiFieldRowThree">
                        ...
                        <input type="text" onChange={handleInput} placeholder="Ingrese ciudad y estado..." name="ubicacion" value={projectDetails.ubicacion}></input>
                    </div>
                    ...
                    <div className="multiFieldRowThree">
                        <input type="text" onChange={handleInput} placeholder="Ingrese giro de la empresa..." name="giro" value={projectDetails.giro}></input>
                    </div>
                    {/* CLIENTE */}
                    ...
                    {/* NEW OCT15 */}
                    {/* <input type="file" name="file1" multiple onChange={handleFileChange} />
                    <input type="file" name="file2" multiple onChange={handleFileChange} />
                    ...
                    {/* <div className="fileCheckbox">
                        <input className="fileBox" type="checkbox" id="checkCIF" onClick={cifFunction()}/> 
                    ...
                    {/* <div>
                        ...
                        />
                        <label>Agree on terms and conditions</label>
                    </div>
                    ...
                    <div className="one">
                        ...
                        <div>
                            ...
                            <div>
                                {checkedCIF.map((item, index) => {
                                    return (
                                        <div className="chip">
                                            <input type="file" name="fileCIF" accept=".pdf" onChange={(e) => setFileCIF(e.target.files)} /> 
                                            {/* <input type="file" name="fileCIF" accept=".pdf" onChange={handleInput} />  */}
                                            {/* <input multiple type="file" name="files[]" accept=".pdf" onChange={(e) => setFileCIF(e.target.files)} />  */}
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <div>
                            ...
                            <div>
                                {checkedCSF.map((item, index) => {
                                    return (
                                        <div className="chip">
                                            <input type="file" name="fileCSF" accept=".pdf" onChange={(e) => setFileCSF(e.target.files)} />
                                            {/* <input type="file" name="fileCSF" accept=".pdf" onChange={handleInput} /> */}
                                            {/* <input multiple type="file" name="files[]" accept=".pdf" onChange={(e) => setFileCSF(e.target.files)} />   */}
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <div>
                            <div className="card-body">
                                {cartaConformidad_items.map((item) => {
                                    return (
                                        <div key={item.id} className="checkbox-container">
                                            <input 
                                                multiple
                                                ...
                                                type="checkbox"
                                                // name="documentos"
                                                name="fileCartaConformidad"
                                                id={item.id}
                                                value={item.value}
                                                onChange={handleSelectCartaConformidad}
                                            />
                                            <label className="checklistLabel"for={item.id}>{item.value}</label>
                                        </div>
                                    )
                            ...
                            <div>
                                {checkedCartaConformidad.map((item, index) => {
                                    return (
                                        <div className="chip">
                                            <input type="file" name="fileCartaConformidad" accept=".pdf" onChange={(e) => setFileCartaConformidad(e.target.files)} /> 
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <div>
                            ...
                            <div>
                                {checkedCotizacion.map((item, index) => {
                                    return (
                                        <div className="chip">
                                            <input type="file" name="fileCotizacion" accept=".pdf" onChange={(e) => setFileCotizacion(e.target.files)} /> 
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <div>
                            ...
                            <div>
                                {checkedFactura.map((item, index) => {
                                    return (
                                        <div className="chip">
                                            <input type="file" name="fileFactura" accept=".pdf" onChange={(e) => setFileFactura(e.target.files)} /> 
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <div>
                                    ...
                                    return (
                                        <div key={item.id} className="checkbox-container">
                                            <input 
                                                ...
                                                type="checkbox"
                                                // name="documentos"
                                                name="fileOrdenCompra"
                                                id={item.id}
                                                value={item.value}
                                                onChange={handleSelectOrdenCompra}
                                            />
                                            <label className="checklistLabel"for={item.id}>{item.value}</label>
                                        </div>
                                    )
                            ...
                            <div>
                                {checkedOrdenCompra.map((item, index) => {
                                    return (
                                        <div className="chip">
                                            <input type="file" name="fileOrdenCompra" accept=".pdf" onChange={(e) => setFileOrdenCompra(e.target.files)} /> 
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <div>
                            ...
                            <div>
                                {checkedOtro.map((item, index) => {
                                    return (
                                        <div className="chip">
                                            <input type="file" name="fileOtro" accept=".pdf" onChange={(e) => setFileOtro(e.target.files)} /> 
                                        </div>
                                    )
                                })}
                            </div>
